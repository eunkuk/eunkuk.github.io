...
tags: [retrospective, migration, spring-boot, php, lessons-learned, 1-person-dev, series]
...
- 문제: PHP → Spring Boot 전환이 단순 코드 번역이 아니라 레거시 호환, 기획 보완, 관리자 웹, 인프라 재구축까지 포함한 작업이었다.
- 맡은 범위: 레거시 분석, 도메인 재설계, 관리자 웹 직접 개발, FE API 조율, 인증/보안/캐시/감사 추적, 문서화.
- 결과: 6모듈 구조와 DomainService + Facade를 세우고, Swagger/Caffeine/Envers를 도입했으며 새 플랜을 판매할 수 있는 상태까지 만들었다.
...
상급자가 IT 직군이 아니었기 때문에, 기술적 결정을 기술 용어가 아니라 비즈니스 영향으로 번역해 설명해야 했다.
