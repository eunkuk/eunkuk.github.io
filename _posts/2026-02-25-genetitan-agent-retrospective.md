...
description: "전임자가 남긴 자동화 스크립트를 연구원이 직접 쓰는 운영 도구로 바꾸기 위해, Tauri + Rust + React 데스크톱 앱으로 재구성한 경험을 정리한다."
---

## 빠른 요약

- 문제: Linux 서버의 `array_db.py`는 자동화 로직은 있었지만, 연구원이 직접 쓰기에는 진행률, 설정, 문서, Windows 지원이 부족했다.
- 맡은 범위: 기존 파이프라인 분석, 현장 워크플로우 재설계, Tauri + Rust + React 데스크톱 앱 구조 설계와 구현.
- 결과: 수동 SCP/SSH/로그 확인 흐름 대신 워크스테이션에서 직접 실행하고 진행률을 확인하는 운영 도구로 전환했다.

## 상황

[마이그레이션 회고](/posts/php-codeigniter-spring-boot-migration-why/)에서 PHP → Spring Boot 전환을 다뤘다. 어드민을 개발하면서 유전자 데이터 업로드 과정이 눈에 들어왔다. 연구원들이 **Axiom Analysis Suite**(Thermo Fisher가 제공하는 GUI 분석 도구)로 마이크로어레이 분석을 수행하고, 결과 파일을 어드민에 수동 업로드하는 구조였다. 업로드를 받는 어드민은 새로 만들었는데, 그 앞단 — Suite로 분석하고, 결과를 정리해서, 업로드하는 과정 — 은 여전히 수작업이었다.

유전자검사 서비스를 운영하고 있다. 검체가 랩에 도착하면 마이크로어레이 칩(Axiom_APMRA_24)에 올리고, GeneTitan 장비가 CEL 파일을 뽑아낸다. 여기까지는 장비가 알아서 한다. CEL 파일이 나오면 연구원이 Suite를 열어서 분석을 돌리고, 결과가 나오면 어드민에 올리는 흐름이었다.

그러던 중 서버에서 `array_db.py`라는 Python 스크립트를 발견했다. 전임 개발자가 만든 것이었다. 코드를 뜯어보니 APT(Affymetrix Power Tools)를 순차 호출해서 분석 파이프라인을 자동화하는 구조였다. Suite가 GUI로 해주는 것을 CLI 레벨에서 스크립트로 돌리는 방식이었다. 하지만 **전임자가 퇴사하면서 사용법이 유실**된 상태였고, 연구원들은 다시 Suite로 돌아가서 수동으로 분석하고 있었다.

코드를 분석해보니 이런 파이프라인이 들어 있었다.

```
CEL 파일 → QC 분석 → DQC 필터링 (≥0.82) → Step1 Genotyping →
Call Rate 필터링 (≥97%) → Plate QC → Step2 Genotyping →
R/SNPolisher (ps-metrics, ps-classification) → VCF 생성 → VCF 필터링 →
Cluster Plot Visualization
```

APT 호출, 임계값 기반 필터링, R 스크립트 실행까지 — Suite에서 수동으로 하던 분석 과정이 스크립트 하나에 들어 있었다. **이 코드를 활용하면 자동화할 수 있지 않을까?**

## 자동화 스크립트는 이미 있었다

전임 개발자가 만든 자동화 도구가 이미 있었다. `array_db.py`는 APT 바이너리를 순차 호출하고, TSV 출력을 파싱해서 필터링하고, R 스크립트로 후처리하는 — 파이프라인 전체를 자동화하는 스크립트였다. 코드 자체는 돌아갔다. 문제는 그대로 가져다 쓸 수 없었다는 것이다.

### 블랙박스 실행

스크립트를 실행하면 터미널에 로그가 흘러간다. 지금 어느 단계인지, 몇 퍼센트 진행됐는지, 어떤 샘플이 필터링됐는지 — 실시간으로 알 수 있는 방법이 없었다. 끝날 때까지 터미널을 보고 있거나, 끝나고 나서 로그 파일을 뒤져봐야 했다.
...
`array_db.py`는 Linux 서버에서만 돌아갔다. APT 바이너리가 Linux 전용이었기 때문이다. 그런데 GeneTitan 장비에 연결된 워크스테이션은 Windows다. 연구원들이 Suite를 쓰는 이유 중 하나도 이것이었다 — Suite는 Windows에서 돌아가니까.

### 사용법 문서 없음

전임자가 퇴사하면서 스크립트의 사용법이 함께 사라졌다. 어떤 인자를 넘겨야 하는지, 라이브러리 파일은 어디에 놓아야 하는지, R 스크립트 경로는 어떻게 설정하는지 — 코드를 직접 읽어서 파악해야 했다.
...
| Windows 미지원 | 워크스테이션이 Windows인데 스크립트는 Linux 전용 |
| 사용법 유실 | 전임자 퇴사로 운영 지식 단절 |

> 자동화 코드는 이미 있었다. 하지만 코드가 있다는 것과 **쓸 수 있다**는 것은 다르다. `array_db.py`의 자동화 로직은 살릴 가치가 있었지만, 연구원이 직접 쓸 수 있는 형태는 아니었다.

## 왜 데스크톱 앱인가

`array_db.py`의 자동화 로직을 살리면서, Suite 대신 연구원이 직접 쓸 수 있는 도구를 만들어야 했다. 스크립트를 리팩토링하거나, 웹 UI를 붙이거나, 서버에서 돌리는 구조를 유지하면서 모니터링만 추가하는 방법도 있었다. 데스크톱 앱을 만들기로 한 건 현장의 조건 때문이었다.
...
현재 자동화된 테스트는 아직 없다. 파이프라인의 각 단계가 외부 바이너리(APT)를 실행하는 구조라서, 단위 테스트를 작성하려면 APT 출력을 모킹해야 한다. 필터링 유틸리티 함수는 테스트할 수 있지만, 파이프라인 전체 흐름은 실제 CEL 파일 없이는 검증할 수 없다.

지금은 수동 테스트로 버티고 있다. 규모가 커지면 통합 테스트 환경을 구축해야 한다.

## 돌아보며

`array_db.py`에서 GeneTitan Agent로의 전환은 **"돌리는 도구"에서 "운영하는 도구"로의 전환**이었다. 동시에, 전임자가 남긴 자동화 코드의 가치를 발견하고, 그것을 연구원이 직접 쓸 수 있는 형태로 발전시킨 과정이기도 했다.
...
`array_db.py`는 전임자가 퇴사하면서 사장될 뻔한 코드였다. 연구원들은 Suite로 돌아갔고, 스크립트는 서버에 남아 있었다. 그 코드를 발견하고, 자동화 로직을 살려서, 연구원이 직접 쓸 수 있는 데스크톱 앱으로 만든 것 — 그게 GeneTitan Agent다. 검체가 들어올 때마다 터미널을 열고 명령어를 타이핑하는 대신, 버튼 하나로 파이프라인을 돌리고 실시간으로 진행률을 확인할 수 있게 된 것. 그게 이 앱을 만든 이유다.
