---
title: "레거시 혼합 환경에서 테스트 전략 다시 세우기 — ArchUnit, E2E, H2 전환기"
date: 2026-03-08 22:00:00 +0900
categories: [Testing]
tags: [spring-boot, testing, tdd, archunit, e2e-test, mybatis, h2, legacy, retrospective]
description: "MyBatis + JPA 혼합 환경에서 H2 테스트, 계층 위반, MockMvc 한계를 겪은 뒤 정착한 테스트 전략을 정리한다."
---

> MyBatis + JPA 혼합 환경에서 H2 테스트가 깨지고, Controller가 계층을 우회하고, MockMvc 테스트는 실제 버그를 충분히 잡지 못했다. 그 경험 이후 정착한 테스트 전략을 기록한다.

---

## 빠른 요약

- 문제: MyBatis + JPA 혼합 환경에서 H2 테스트, 계층 위반, `@WebMvcTest`의 한계가 동시에 드러났다.
- 맡은 범위: 테스트 프로파일 정리, 레거시 스키마 주입, ArchUnit 규칙 도입, E2E 중심 테스트 전략 재정립.
- 결과: 설정 누락, 계층 위반, 보안/예외 처리 누락 같은 실제 버그를 테스트에서 먼저 잡는 구조로 바꿨다.
...
이건 추적 비용이 큰 유형의 버그다. 로컬에서는 잘 되고, 테스트에서만 터진다. 원인은 설정 파일 누락.
...
문제는 이게 **잘 동작한다**는 것이다. 컴파일도 되고, 런타임 에러도 없다. 코드 리뷰에서도 눈에 띄지 않은 채 넘어가기 쉽다.
